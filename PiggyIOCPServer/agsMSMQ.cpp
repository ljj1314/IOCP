...
{		
	...
	QPropVar[dwIDprop].bVal  = MQMSG_DELIVERY_EXPRESS;//MQMSG_DELIVERY_RECOVERABLE;
	dwIDprop++;   
}
...
BOOL agsMSMQ::OpenMQ(CString serv, CString pathmsg, CString labelmsg, CString msg)
{
	//m_sPathQ.Format(_T("%s\\PRIVATE$\\%s"),serv,pathmsg);    
	m_sPathQ.Format(_T("DIRECT=TCP:%s\\PRIVATE$\\%s"),serv,pathmsg);
	::MultiByteToWideChar(CP_ACP,0,m_sPathQ,-1,(LPWSTR)wszPath,sizeof(wszPath));
...
BOOL agsMSMQ::SendMQ(CString serv, CString pathmsg, CString labelmsg, CString msg)
{
	unsigned char	pMsg[2048]={0};
	strcpy((char*)pMsg,msg);
	return SendMQ(serv,pathmsg,labelmsg,msg,pMsg);
}

BOOL agsMSMQ::SendMQ(CString serv, CString pathmsg,CString labelmsg,CString msg,unsigned char* msgbuf)
{
	dwIDprop = 0;
	QMsg[dwIDprop]					= PROPID_M_BODY;
	QPropVar[dwIDprop].vt			= VT_VECTOR | VT_UI1;	
	QPropVar[dwIDprop].caub.pElems	= msgbuf;
	QPropVar[dwIDprop].caub.cElems	= msg.GetLength();//strlen(msg);
	dwIDprop++;
	//InitialMQMSG();
	QPropMsg.cProp		= dwIDprop;
	QPropMsg.aPropID	= QMsg;
	...
	{
		m_sError = "Error to send private queue";
		OpenMQ(serv,pathmsg,labelmsg,msg);
		return FALSE;
	}
...
BOOL agsMSMQ::CloseMQ(CString serv, CString pathmsg, CString labelmsg, CString msg)
{
	hr = MQCloseQueue(hQue);
	if(FAILED(hr))
	{
